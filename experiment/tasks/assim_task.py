...
from scripts.omf import main as monitoring_plots
...
class AnalysisMonitoring(AbstractTask):
    """Perturb state task."""
    ...
    def __init__(self, config):
        ...
        """
        AbstractTask.__init__(self, config, name="AnalysisMonitoring")
        self.var_name = self.config.get_value("task.var_name")
        try:
            user_config = self.config.get_value("task.forcing_user_config")
        except AttributeError:
    ...
    def execute(self):
        ...
        output_dir = self.config.get_value("system.sfx_exp_data") + "/archive/monitoring/" + dtg.strftime("%Y/%m/%d/%H/")
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)

        stream = self.config.get_value("general.times.start")[:4]
        cfgfile = self.config.get_value("metadata.source_file_path")
